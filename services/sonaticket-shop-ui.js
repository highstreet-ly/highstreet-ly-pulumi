...
const shopVersion = config.require("sonaticket-shop-version");
...
module.exports = function (settings, ingressGenerator) {
    ...
    return {
        replicas: 1,
        name: 'sonaticket-shop',
        containerName: `highstreet-tickets-ui${uiImagePostpend}`,
        bc: 'ui',
        containerVersion: shopVersion,
        hasIngress: true,
        usesVault: false,
        settings: settings.dashBffSettings,
        createIngress: function (deployment) {
            ingressGenerator.createIngress('shop.', 'sonaticket-shop', 80, [
                {
                    path: ''
                },
                {
